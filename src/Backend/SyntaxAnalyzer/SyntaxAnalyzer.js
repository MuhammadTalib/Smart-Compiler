  ...
  if (t[i].CP === "while") {
    ...
    if (t[i].CP === "(") {
      ...
      if (EXP(T, scope, null, tname)) {
        ...
        if (!Tr) {
          console.log("Condition Type Mismatch at line", t[i].line);
        }
        if (t[i].CP === ")") {
          i++;
...
function FOR_PARAM_2(N, scope) {
  if (t[i].CP === ";") {
    if (!ST.lookupST(N)) {
      console.log(
        "Undeclared Error | Variable ",
        N,
        " not declared at line ",
        t[i].line
      );
    }
    i++;
    ...
    if (t[i].CP === "ID") {
      ...
      if (CONST_DT1(Tr, scope, tICG)) {
        ...
        if (ref === null && !ST.insertST(N, Tr.current, scope, null)) {
          ...
          ref !== null &&
          !ST.insertCDT(N, Tr.current, "public", null, ref)
        ) {
          console.log(
            "Redeclaration Error | variable ",
            N,
            ...
            if (MST(scope, RT)) {
              ...
              if (t[i].CP === "}") {
                ...
                if (!ST.insertCT(N, T, null, null)) {
                  console.log(
                    "Redeclaration Error | Redeclaring Function ",
                    N,
                    " at line ",
                    t[i].line
                  );
      ...
      if (DEC1(T, scope, ref, tICG)) {
        if (ref !== null) {
          if (!ST.insertCDT(N, T, "public", null, ref)) {
            console.log(
              "Redeclaration Error | Class Attribute at Line :",
              t[i].line,
              "Variable",
              N
            );
  ...
  if (find(DEC11, t[i].CP)) {
    ...
    if (DEC2(Tr, scope, ref, t2ICG)) {
      ...
      if (!ST.compatibility(Tl, Tr.current, O)) {
        console.log(
          "Mismatch Assigning Type | Can't Assign ",
          Tl,
          " variable to ",
          Tr.current,
          " type"
...
function S_DASH(Tl, T, scope, ref, tICG, t2ICG) {
  if (t[i].CP === "ROP") {
    ...
    if (R(Tr, scope, ref, TrICG)) {
      ...
      TaICG.current = createTemp();
      Output(
        TaICG.current + " = " + t2ICG.current + " " + O + " " + TrICG.current
      );
      var Ta = React.createRef();
      Ta.current = ST.compatibility(Tl.current, Tr.current, O);
  ...
  if (t[i].CP === "(") {
    ...
    if (F(Tl, scope, ref, tICG)) {
      if (!ST.compatibility("int", Tl.current, "+")) {
        console.log(
          "Can't increment decrement variable of ",
          Tl.current,
          " type"
        );
      }
...
function NEW_ASGN(N, T, scope, ref, CN, t2ICG) {
  if (t[i].CP === "inc_dec") {
    ...
    if (!T.current) {
      console.log(
        "Type Mismatch | increment decrement can't be applied on Type of ",
        Tl
      );
    }
    return true;
