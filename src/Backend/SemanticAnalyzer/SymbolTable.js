...
export default class SymbolTable {
  ...
  lookupFT(N) {
    for (var j = 0; j < this.ClassTable.length; j++) {
      if (
        this.ClassTable[j].Name === N &&
        this.ClassTable[j].Type !== "class"
      ) {
  ...
  checkCompatibilityOfPL(PL, EPL, F) {
    if (PL.length !== EPL.length) {
      return console.log("Invalid Parameter length passed to Function ", F);
    } else
      for (var i = 0; i < PL.length; i++) {
        if (!this.compatibility(PL[i], EPL[i], "=")) {
          console.log(
            "Type Mismath | Invalid parameter ",
            i + 1,
            " passed to Function ",
            F
          );
  ...
  lookupCDT_Functions(N, PL, ref, access) {
    if (ref.current) {
      for (var i = 0; i < ref.current.length; i++) {
        if (
          ref.current[i].Name === N &&
          ref.current[i].Type.split("-")[0] === PL &&
          ref.current[i].AM === access
        ) {
  ...
  compatibility(T1, T2, Op) {
    ...
    for (var i = 0; i < compatibilityArray.length; i++) {
      if (
        compatibilityArray[i].T1 === T1 &&
        compatibilityArray[i].T2 === T2 &&
        compatibilityArray[i].O === Op
      ) {
